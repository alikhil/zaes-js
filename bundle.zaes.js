...
},{"./src/aes":10,"./src/utils":11}],2:[function(require,module,exports){
'use strict'
...
},{"_process":7,"buffer":4,"crypto":3}],9:[function(require,module,exports){
"use strict";
...
var S_BOX = [];

module.exports.R_CON = [[0x00, 0x00, 0x00, 0x00], [0x01, 0x00, 0x00, 0x00], [0x02, 0x00, 0x00, 0x00], [0x04, 0x00, 0x00, 0x00], [0x08, 0x00, 0x00, 0x00], [0x10, 0x00, 0x00, 0x00], [0x20, 0x00, 0x00, 0x00], [0x40, 0x00, 0x00, 0x00], [0x80, 0x00, 0x00, 0x00], [0x1b, 0x00, 0x00, 0x00], [0x36, 0x00, 0x00, 0x00]];
...
function getBlockFromState(state) {
	var block = new Array(a_u.BLOCK_LENGTH);
	for (var i = 0; i < state.length; i++) {
		for (var j = 0; j < state[i].length; j++) {
...
function applyActionToBlocks(action, blocks, keySchedule) {
	var newBlocks = new Array(blocks.length);
	for (var i = 0; i < blocks.length; i++) {
...
function expandKey(key) {
	var tempWord = new Array(WORD_LENGTH);
	var rounds = getRoundsCount(key);
	var wordsCount = key.length / WORD_LENGTH;
	var keySchedule = new Array(COLUMNS * (rounds + 1));

	for (var i = 0; i < wordsCount; i++) {
		keySchedule[i] = key.slice(WORD_LENGTH * i, WORD_LENGTH * i + WORD_LENGTH);
	}
	for (var _i = wordsCount; _i < keySchedule.length; _i++) {
		tempWord = keySchedule[_i - 1];
		if (_i % wordsCount === 0) {
			tempWord = a_u.xorWords(a_u.subWord(a_u.rotWordLeft(tempWord)), a_u.R_CON[_i / wordsCount]);
		} else if (wordsCount > 6 && _i % wordsCount === 4) {
			tempWord = a_u.subWord(tempWord);
		}
		keySchedule[_i] = a_u.xorWords(keySchedule[_i - wordsCount], tempWord);
	}
	return keySchedule;
}
 ...
 * @param key AES key generated by generateKey function, should have length 16, 24 or 32(128, 192 or 256 bits)
 ...
 * @param bytes Array of bytes
 * @param key Key with which array is encrypted. Should have length 16, 24 or 32(128, 192 or 256 bits)
...
module.exports.stringToBytes = function (str) {
	var bytesPerChar = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
...
module.exports.bytesToString = function (bytes) {
	...
	var str = "";
	var iter = 0;
	var strLength = bytes.length / bytesPerChar;
